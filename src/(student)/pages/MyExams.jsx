...
export default function MyExams({ endPointParams }) {
  ...
  const [examsAnswersID, setExamsAnswersID] = useState(null);
  ...
  const fetchExamsAnswersID = async () => {
    try {
      // dapetin exam ID
      const response1 = await getData(`${endPointParams}?class_id=${post.id}`);
      const exams = response1[0];
      setExamsID(exams.id);

      const response = await getData(`${endPointParams}-answers?exam_id=${exams.id}&student_id=${studentID}`);

      if (Array.isArray(response) && response.length > 0) {
        setExamsAnswersID(response[0].id);
        setSelectedAnswers(response[0].answers || {});
        setAnsweredQuestions(Object.keys(response[0].answers || {}).map(key => parseInt(key)));
      } else {
        // buat exam answer, nanti dapet ExamAnswersID
        const newAnswer = await postData(`${endPointParams}-answers`, {
          exam_id: exams.id,
          student_id: studentID,
          answers: {},
        });
        setExamsAnswersID(newAnswer.id);
      }
    } catch (err) {
  ...
  const fetchAllQuestions = async () => {
    ...
    while (!stop) {
      try {
        const response = await getData(`${endPointParams}/student?class_id=${post.id}&number=${number}`);
  ...
  const handleAnswer = async (number, value) => {
    ...
    try {
      await updateData(`${endPointParams}-answers?id=${examsAnswersID}`, {
        exam_id: examsID,
        student_id: studentID,
        answers: updated,
      });
  ...
  return (
    <div className="flex gap-8 ml-[22rem] mr-24 my-16">
      ...
      {showConfirmModal && (
        <Modal
          title="Confirmation of Exam Submission"
          fields={[{ customComponent: <p>Are you sure you want to submit your exam?</p> }]}
          children="Submit"
